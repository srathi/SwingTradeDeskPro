...
def _ensure_rsi(df, feature_cols, period: int = 14):
    """Compute a Wilder RSI column in-place if a 7-channel tokenizer needs it."""
    if "rsi" in feature_cols and "rsi" not in df.columns:
        ...
        df["rsi"] = rsi.bfill().ffill().fillna(50.0).values
...
class KronosPredictor:
    ...
    def __init__(self, model, tokenizer, device=None, max_context=512, clip=5):
        ...
        # Input channels fed to the tokenizer (defaults to the 6 Kronos channels).
        # Set to CHANNELS_7 (open,high,low,close,volume,amount,rsi) for a
        # fine-tuned 7-channel (RSI) tokenizer.
        self.feature_cols = self.price_cols + [self.vol_col, self.amt_vol]
        # Channels actually predicted/returned (RSI is an input feature, not output).
        self.output_cols = self.price_cols + [self.vol_col, self.amt_vol]
    ...
    def predict(self, df, x_timestamp, y_timestamp, pred_len, T=1.0, top_k=0, top_p=0.9, sample_count=1, verbose=True):
        ...
        _ensure_rsi(df, self.feature_cols)
        ...
        # Drop any trailing non-output channels (e.g. the RSI input channel).
        n_out = len(self.output_cols)
        preds = preds[:, :n_out] * (x_std[:n_out] + 1e-5) + x_mean[:n_out]

        pred_df = pd.DataFrame(preds, columns=self.output_cols, index=y_timestamp)
        return pred_df
    ...
    def predict_batch(self, df_list, x_timestamp_list, y_timestamp_list, pred_len, T=1.0, top_k=0, top_p=0.9, sample_count=1, verbose=True):
        ...
        n_out = len(self.output_cols)
        for i in range(num_series):
            preds_i = preds[i][:, :n_out] * (stds[i][:n_out] + 1e-5) + means[i][:n_out]
            pred_df = pd.DataFrame(preds_i, columns=self.output_cols, index=y_timestamp_list[i])

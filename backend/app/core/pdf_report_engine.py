...
class PDFReportEngine:
    ...
    def _create_styles(cls) -> Dict[str, ParagraphStyle]:
        ...
        custom_styles = {
            'DocTitle': ParagraphStyle('DocTitle', parent=styles['Heading1'], fontName='Helvetica-Bold', fontSize=15, leading=18, textColor=colors.HexColor('#0f172a')),
            'SubTitle': ParagraphStyle('SubTitle', parent=styles['Normal'], fontName='Helvetica', fontSize=9, leading=12, textColor=colors.HexColor('#475569')),
            'SectionHeading': ParagraphStyle('SectionHeading', parent=styles['Heading2'], fontName='Helvetica-Bold', fontSize=11, leading=14, textColor=colors.HexColor('#0f172a'), spaceBefore=8, spaceAfter=4),
    ...
    def generate_deepscan_pdf(cls, data: Dict[str, Any]) -> bytes:
        ...
        day_change = float(data.get('day_change_pct', 0.0))
        sector = data.get('sector', 'N/A')
        fusion = data.get('alpha_fusion', {})
        score = fusion.get('composite_score', 50)
        ...
        header_data = [
            [
                ...
                Paragraph(f"<font size=16 color='#0284c7'><b>₹{cmp:,.2f}</b></font><br/><font size=8 color='{'#16a34a' if day_change >= 0 else '#dc2626'}'><b>{'+' if day_change >= 0 else ''}{day_change:.2f}% (Day)</b></font>", styles['TableCellBold'])
            ],
            [
                Paragraph(f"<b>Symbol:</b> {ticker} &nbsp;|&nbsp; <b>Sector:</b> {sector} &nbsp;|&nbsp; <b>Exchange:</b> NSE / BSE", styles['SubTitle']),
        ...
        mtf_table_data = [
            ...
            [
                ...
                Paragraph(f"Close: ₹{s1.get('close', cmp)} | 13 EMA: ₹{s1.get('ema_13', 0)} | 26 EMA: ₹{s1.get('ema_26', 0)}", styles['TableCell'])
            ],
            [
                ...
                Paragraph(f"20 EMA: ₹{s2.get('ema_20', 0)} | 50 EMA: ₹{s2.get('ema_50', 0)} | 200 EMA: ₹{s2.get('ema_200', 0)}", styles['TableCell'])
            ],
            [
                ...
                Paragraph(f"RSI(14): {s3.get('rsi_14', 50)} | Vol Ratio: {s3.get('vol_ratio', 1.0)}x | Candle: {'Green' if s3.get('is_green_candle') else 'Red'}", styles['TableCell'])
            ]
        ]
        ...
        active_strats = data.get('active_strategies', [])
        primary = None
        ...
        if primary:
            strat_name = primary.get('strategy', 'Quantitative Model')
            ...
            strat_table_data = [
                ...
                [
                    ...
                    Paragraph(f"<b>Entry:</b> ₹{entry:,.2f}<br/><b>Stop Loss:</b> ₹{sl:,.2f} <font color='#dc2626'>(-{risk_pct:.1f}%)</font>", styles['TableCell']),
                    Paragraph(f"<b>Target 1 (2R):</b> ₹{t1:,.2f} <font color='#16a34a'>(+{gain_t1:.1f}%)</font><br/><b>Target 2 (3R):</b> ₹{t2:,.2f} <font color='#16a34a'>(+{gain_t2:.1f}%)</font>", styles['TableCell']),
        ...
        # --- SECTION 3: INSTITUTIONAL POSITION SIZING (₹5,00,000 Capital, 1% Risk) ---
        story.append(Spacer(1, 8))
        story.append(Paragraph("3. Institutional Position Sizing (Fixed ₹5,00,000 Capital @ 1.0% Risk)", styles['SectionHeading']))
        ...
        pos_table_data = [
            ...
            [
                Paragraph("₹5,00,000", styles['TableCellBold']),
                Paragraph("₹5,000", styles['TableCellBold']),
                Paragraph(f"<b>{sizing.get('shares', 0)} shares</b>", styles['TableCellCyan']),
                Paragraph(f"₹{sizing.get('total_capital_deployed', 0):,.2f}", styles['TableCell']),
                Paragraph(f"<font color='#16a34a'><b>+₹{sizing.get('potential_profit_target_1', 0):,.2f}</b></font>", styles['TableCellBold'])
            ]
        ]
        ...
        ti_data = [
            ...
            [
                Paragraph("20 EMA (Short-Term Trend)", styles['TableCellBold']), Paragraph(f"₹{data.get('ema_20', cmp):,.2f}", styles['TableCell']),
                Paragraph("RSI (14-Day Momentum)", styles['TableCellBold']), Paragraph(f"{data.get('rsi_14', 50.0):.1f} ({'Overbought' if data.get('rsi_14', 50) > 70 else 'Oversold' if data.get('rsi_14', 50) < 30 else 'Neutral Momentum'})", styles['TableCell'])
            ],
            [
                Paragraph("50 EMA (Medium-Term Base)", styles['TableCellBold']), Paragraph(f"₹{data.get('ema_50', cmp):,.2f}", styles['TableCell']),
                Paragraph("ATR (14-Day Volatility)", styles['TableCellBold']), Paragraph(f"₹{data.get('atr_14', cmp * 0.02):,.2f} ({((data.get('atr_14', cmp * 0.02)/cmp)*100):.1f}%)", styles['TableCell'])
            ],
            [
                Paragraph("200 EMA (Macro Institutional Line)", styles['TableCellBold']), Paragraph(f"₹{data.get('ema_200', cmp):,.2f}", styles['TableCell']),
                Paragraph("Volume Ratio (vs 20D SMA)", styles['TableCellBold']), Paragraph(f"{data.get('vol_ratio', 1.0):.2f}x ({'Heavy Surge' if data.get('vol_ratio', 1.0) >= 1.3 else 'Normal Volume'})", styles['TableCell'])
            ],
            [
                Paragraph("Volume-Weighted Avg Price (VWAP)", styles['TableCellBold']), Paragraph(f"₹{data.get('vwap', cmp):,.2f}", styles['TableCell']),
                Paragraph("52-Week Range", styles['TableCellBold']), Paragraph(f"₹{data.get('low_52w', cmp*0.8):,.2f} - ₹{data.get('high_52w', cmp*1.2):,.2f}", styles['TableCell'])
            ]
        ]
        ...
        if kronos and kronos.get('trajectory'):
            traj = kronos.get('trajectory', [])
            end_price = float(kronos.get('target_price', traj[-1]['predicted_close'] if traj else cmp))
            p10 = float(kronos.get('overall_projected_low', min([x.get('band_low', cmp) for x in traj]) if traj else cmp*0.95))
            p90 = float(kronos.get('overall_projected_high', max([x.get('band_high', cmp) for x in traj]) if traj else cmp*1.05))
            expected_ret = float(kronos.get('target_pct_change', (((end_price - cmp) / cmp) * 100.0) if cmp > 0 else 0.0))
            bias_dir = kronos.get('direction', 'BULLISH')
            ...
            kronos_table = [
                ...
                [
                    Paragraph(f"<b>₹{end_price:,.2f}</b>", styles['TableCellCyan']),
                    ...
                    Paragraph(f"₹{p10:,.2f} &nbsp;to&nbsp; ₹{p90:,.2f} (Spread: ₹{(p90-p10):,.2f})", styles['TableCell']),
            ...
            story.append(Paragraph("<i>Kronos AI model trajectory awaiting next scheduled batch inference.</i>", styles['TableCell']))
        ...
        macro_table_data = [
            ...
            [
                ...
                Paragraph("USD / INR Parity", styles['TableCellBold']), Paragraph(f"₹{macro_hud.get('usd_inr_rate', 84.15):.2f}", styles['TableCell'])
            ]
        ]
    ...
    def generate_backtest_pdf(cls, metrics: Dict[str, Any]) -> bytes:
        ...
        header_data = [
            [
                ...
                Paragraph(f"<font size=15 color='{'#16a34a' if net_profit >= 0 else '#dc2626'}'><b>{'+' if net_profit >= 0 else ''}₹{net_profit:,.2f}</b></font><br/><font size=8 color='{'#16a34a' if net_profit_pct >= 0 else '#dc2626'}'><b>({'+' if net_profit_pct >= 0 else ''}{net_profit_pct:.2f}% Return)</b></font>", styles['TableCellBold'])
            ],
            [
                Paragraph(f"<b>Strategy:</b> {strat_id} &nbsp;|&nbsp; <b>Symbol/Basket:</b> {ticker} &nbsp;|&nbsp; <b>Horizon:</b> {period}", styles['SubTitle']),
                Paragraph(f"<b>Final Equity:</b> ₹{final_cap:,.2f} &nbsp;|&nbsp; <b>CAGR:</b> {cagr:.1f}%", styles['TableCellBold'])
            ]
        ]
        t_head = Table(header_data, colWidths=[380, 160])
        t_head.setStyle(TableStyle([
        ...
        scorecard_data = [
            ...
            [
                Paragraph("Initial Capital", styles['TableCellBold']), Paragraph(f"₹{init_cap:,.2f}", styles['TableCell']),
                Paragraph("Total Closed Trades", styles['TableCellBold']), Paragraph(f"{total_trades} trades", styles['TableCell'])
            ],
            [
                Paragraph("Net PnL", styles['TableCellBold']), Paragraph(f"<font color='{'#16a34a' if net_profit >= 0 else '#dc2626'}'><b>₹{net_profit:,.2f}</b></font>", styles['TableCell']),
        ...
        for tr in trades[:14]:
            ...
            trade_rows.append([
                ...
                Paragraph(f"₹{tr.get('entry_price', 0):,.1f}", styles['TableCell']),
                Paragraph(f"₹{tr.get('exit_price', 0):,.1f}", styles['TableCell']),
                Paragraph(f"<font color='{'#16a34a' if pnl >= 0 else '#dc2626'}'><b>₹{pnl:,.1f}</b></font>", styles['TableCell']),
    ...
    def generate_macro_pdf(cls, macro_data: Dict[str, Any]) -> bytes:
        ...
        hud_table_data = [
            ...
            [
                ...
                Paragraph("USD / INR Forex Rate", styles['TableCellBold']), Paragraph(f"<b>₹{hud.get('usd_inr', 84.15):.2f}</b>", styles['TableCell'])
            ]
        ]
